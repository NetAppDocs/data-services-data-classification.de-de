...
[role="lead"]
NetApp Data Classification ist ein Data-Governance-Service in der NetApp Console, der Ihre lokalen und Cloud-Datenquellen scannt, um Daten zuzuordnen und zu klassifizieren sowie private Informationen zu identifizieren. Dadurch werden Sicherheits- und Compliance-Risiken reduziert, Speicherkosten gesenkt und Datenmigrationen unterstützt.


IMPORTANT: Ab Version 1.31 ist Datenklassifizierung als Kernfunktion innerhalb der NetApp Console verfügbar. Es fallen keine zusätzlichen Kosten an. Für Datenklassifizierung ist weder eine Lizenz noch ein Abonnement erforderlich. + Wenn bisher die ältere Version 1.30 oder eine frühere Version genutzt wurde, bleibt diese bis zum Ablauf des Abonnements verfügbar.
...
Die NetApp Console ermöglicht die zentrale Verwaltung von NetApp Speicher- und Datendiensten in lokalen und Cloud-Umgebungen im Unternehmensmaßstab. Die Console ist für den Zugriff auf und die Nutzung von NetApp Datendiensten erforderlich. Als Managementoberfläche ermöglicht sie die Verwaltung zahlreicher Speicherressourcen über eine einzige Oberfläche. Console-Administratoren können den Zugriff auf Speicher und Dienste für alle Systeme im Unternehmen steuern.
...
* Personenbezogene Daten (PII) werden identifiziert.
...
Data Classification bietet Werkzeuge zur Senkung der Gesamtbetriebskosten (TCO) von Speichern. Data Classification kann für Folgendes verwendet werden:
...
Die Kosten für den Datentransfer hängen von Ihrer Konfiguration ab. Befinden sich die NetApp Data Classification-Instanz und die Datenquelle in derselben Verfügbarkeitszone und Region, fallen keine Kosten für den Datentransfer an. Befindet sich die Datenquelle, wie zum Beispiel ein Cloud Volumes ONTAP-System, jedoch in einer anderen Verfügbarkeitszone oder Region, berechnet Ihr Cloud-Anbieter Kosten für den Datentransfer. Weitere Einzelheiten bieten die folgenden Links:
...
* In AWS läuft die Datenklassifizierung auf einem https://aws.amazon.com/ec2/instance-types/m6i/["m6i.4xlarge-Instanz"^] mit einer 500 GiB GP2-Festplatte. Das Betriebssystem-Image ist Amazon Linux 2.
...
* **Vollständige Scans** bieten eine tiefgehende Analyse Ihrer Daten. Ein vollständiger Scan umfasst eine reine Kartendarstellung und die Klassifizierung der Daten innerhalb der Dateien.


Eine detaillierte Aufschlüsselung der Unterschiede zwischen reinen Kartenscans und vollständigen Scans finden Sie unter link:task-scanning-overview.html["Was ist der Unterschied zwischen Mapping- und Klassifizierungsscans?"].
