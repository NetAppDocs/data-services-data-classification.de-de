...
* *Nur-Karten-Scans* bieten lediglich einen groben Überblick über Ihre Daten. Das Mapping kann auf Ihren Datenquellen sehr schnell durchgeführt werden, da dabei nicht auf Dateien zugegriffen wird, um die darin enthaltenen Daten anzusehen.
* *Zuordnen und Klassifizieren*-Scans ermöglichen eine gründliche Überprüfung Ihrer Daten.


Möglicherweise profitieren Sie davon, Map-only-Scans zu verwenden, um Ihre Daten schnell zu durchsuchen und die Datenquellen zu identifizieren, die weiterer Recherche bedürfen, und dann später Map- und Klassifizierungsscans nur für diese benötigten Datenquellen oder Volumes zu aktivieren.

Die Priorität wird nur Map-only-Scans zugewiesen, nicht Map-and-classify-Scans.
