...
.Checklist
* Stellen Sie sicher, dass eine Netzwerkverbindung zwischen der Data Classification Instanz und jedem Netzwerk besteht, das Volumes für Cloud Volumes ONTAP oder On-Premises ONTAP Cluster enthält.
...
NOTE: Die Datenklassifizierung scannt nur eine Dateifreigabe unter einem Volume. Wenn Ihre Volumes mehrere Freigaben enthalten, müssen diese anderen Freigaben separat als Freigabegruppe gescannt werden. link:reference-limitations.html#data-classification-scans-only-one-share-under-a-volume["Weitere Einzelheiten zu dieser Datenklassifizierungsbeschränkung"^].
