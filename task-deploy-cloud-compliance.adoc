...
Beachten Sie, dass Sie auch link:task-deploy-compliance-onprem.html["Installieren Sie Data Classification auf einem Linux-Host mit Internetzugang"]. Diese Art der Installation kann eine gute Option sein, wenn bevorzugt lokale ONTAP-Systeme mit einer ebenfalls lokal bereitgestellten Data Classification Instanz gescannt werden sollen, dies ist jedoch keine Voraussetzung. Die Software funktioniert unabhängig von der gewählten Installationsmethode exakt gleich.
...
Wenn Sie noch keinen Konsolenagenten haben, erstellen Sie einen. Sehenlink:https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["Erstellen eines Konsolenagenten in AWS"^] ,link:https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-azure.html["Erstellen eines Konsolenagenten in Azure"^] , oderlink:https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-google.html["Erstellen eines Konsolenagenten in GCP"^] Die

[role="quick-margin-para"]
Sie können auchlink:https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html["Installieren Sie den Konsolen-Agenten vor Ort"^] auf einem Linux-Host in Ihrem Netzwerk oder auf einem Linux-Host in der Cloud.

.image:https://raw.githubusercontent.com/NetAppDocs/common/main/media/number-2.png["Zwei"]Voraussetzungen
[role="quick-margin-para"]
Stellen Sie sicher, dass Ihre Umgebung die Voraussetzungen erfüllt. Dazu gehören ausgehender Internetzugang für die Instanz, Konnektivität zwischen dem Console-Agent und Data Classification über Port 443 und mehr. <<Voraussetzungen,Vollständige Liste anzeigen>>.
...
Falls Sie noch keinen Console Agent besitzen, erstellen Sie einen Console Agent bei Ihrem Cloud-Anbieter. Siehe  https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-aws.html["Erstellen eines Konsolenagenten in AWS"^] oder  https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-azure.html["Erstellen eines Konsolenagenten in Azure"^], oder  https://docs.netapp.com/us-en/console-setup-admin/concept-install-options-google.html["Erstellen eines Konsolenagenten in GCP"^]. In den meisten Fällen ist wahrscheinlich bereits ein Console Agent eingerichtet, bevor versucht wird, Data Classification zu aktivieren, da die meisten  https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html["Für Konsolenfunktionen ist ein Konsolenagent erforderlich"]. Es gibt jedoch Fälle, in denen jetzt einer eingerichtet werden muss.

Es gibt einige Szenarien, in denen ein Console-Agent verwendet werden muss, der bei einem bestimmten Cloud-Anbieter bereitgestellt wird:
...
Beachten Sie, dass Sie auch https://docs.netapp.com/us-en/console-setup-admin/task-install-connector-on-prem.html["Installieren Sie den Konsolen-Agenten vor Ort"^] auf einem Linux-Host in Ihrem Netzwerk oder in der Cloud. Einige Benutzer, die die Datenklassifizierung vor Ort installieren möchten, entscheiden sich möglicherweise auch für die Installation des Konsolenagenten vor Ort.

Es kann Situationen geben, in denen Sie verwenden müssen https://docs.netapp.com/us-en/console-setup-admin/concept-agents.html#multiple-systems-and-agents["mehrere Konsolenagenten"^] Die
...
link:https://docs.netapp.com/us-en/console-setup-admin/task-install-restricted-mode.html["Erfahren Sie mehr über die Bereitstellung des Console-Agenten in einer Regierungsregion"^].
...
* link:https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-resource-limits.html["AWS-Dokumentation: Amazon EC2-Servicekontingente"^]
* link:https://docs.microsoft.com/en-us/azure/virtual-machines/linux/quotas["Azure-Dokumentation: vCPU-Kontingente virtueller Computer"^]
* link:https://docs.cloud.google.com/docs/quotas/view-manage["Google Cloud-Dokumentation: Ressourcenkontingente"^]
