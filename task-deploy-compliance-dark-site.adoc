...
---
= Installation von NetApp Data Classification auf einem Linux-Host im privaten Modus
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
Der _private Modus_ ermöglicht die Installation von NetApp Data Classification auf einem Linux-Host in einer lokalen Umgebung ohne Internetzugang. Bei dieser Bereitstellung wird ein Installationsskript verwendet und die Data Classification-Instanz bleibt von der NetApp Console SaaS-Ebene getrennt, sodass lokale Datenquellen in einer isolierten Umgebung gescannt werden können.
...
Das Installationsskript für die Datenklassifizierung prüft, ob System und Umgebung die erforderlichen Voraussetzungen erfüllen. Sind diese erfüllt, beginnt die Installation. Wenn die Voraussetzungen unabhängig von der Ausführung der Data Classification Installation überprüft werden sollen, steht ein separates Softwarepaket zum Download zur Verfügung, das ausschließlich die Voraussetzungen testet. link:task-test-linux-system.html["Erfahren Sie, wie Sie überprüfen können, ob Ihr Linux-Host für die Installation der Datenklassifizierung bereit ist."].
...
* Die Datenklassifizierung muss auf einem dedizierten Host erfolgen. Der Host darf nicht mit anderen Anwendungen oder Drittanbietersoftware wie Antivirus gemeinsam genutzt werden.
...
** *Azure VM-Größe*: "Standard_D16_v5". link:reference-instance-types.html#azure-instance-types["Weitere Azure-Instanztypen anzeigen"^].
...
+
*** Red Hat Enterprise Linux version 8.8, 8.10, 9.0, 9.1, 9.2, 9.3, 9.4, 9.5, 9.6 und 9.7.


** Advanced Vector Extensions (AVX2) müssen auf dem Hostsystem aktiviert sein.


* *Red Hat Subscription Management*: Der Host muss bei Red Hat Subscription Management registriert sein. Ist keine Registrierung vorhanden, kann das System während der Installation nicht auf die Repositories zugreifen, um erforderliche Drittanbietersoftware zu aktualisieren.
...
NOTE: Das Installationsprogramm für Data Classification reserviert UID 4321 und GID 4321. Vor Beginn der Installation sollte sichergestellt sein, dass UID 4321 und GID 4321 auf dem Host-System nicht verwendet werden. Falls sie verwendet werden, schlägt die Installation mit einer „invalid user“-Fehlermeldung fehl.
...
In den meisten Situationen wird Data Classification auf einem Einzelhostsystem installiert. Dies ist nicht derselbe VM-Host wie der Console Agent.
