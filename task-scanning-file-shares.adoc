...
+
** Die Datenklassifizierung kann weder Berechtigungen noch den "zuletzt aufgerufenen Zeitpunkt" aus 7-Mode Systemen extrahieren.
...
+
Um sicherzustellen, dass die „Last Accessed Times“ Ihrer Dateien durch Data Classification-Scans unverändert bleiben, wird der Active Directory-Benutzer mit Write Attributes-Berechtigungen in CIFS oder mit Schreibberechtigungen in NFS konfiguriert. Wenn möglich, wird der Active Directory-Benutzer als Teil einer übergeordneten Gruppe in der Organisation konfiguriert, die Berechtigungen für alle Dateien besitzt.
...
NOTE: Die Systemübersichtsseite bietet die Option *Scannen ohne Schreibberechtigungen*, die standardmäßig deaktiviert ist. Wenn NetApp Data Classification nicht über die entsprechenden Berechtigungen verfügt, werden die Dateien nicht gescannt, da NetApp Data Classification die „Zuletzt zugegriffen“-Zeit nicht auf den ursprünglichen Zeitstempel zurücksetzen kann. + Diese Einstellung lässt sich mit der Option *Scannen ohne Schreibberechtigungen* überschreiben. Weitere Informationen zu dieser Option sind unter link:reference-collected-metadata.html#last-accessed-time-timestamp["Aus Datenquellen in der Datenklassifizierung gesammelte Metadaten"] zu finden.
