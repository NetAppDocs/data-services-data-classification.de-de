...
---
= Active Directory mit NetApp Data Classification integrieren
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
* Sie können link:task-investigate-data.html["vollständige Dateiberechtigungen"] für jede Datei und jedes Verzeichnis sehen, wenn Sie auf die Schaltfläche „Alle Berechtigungen anzeigen“ klicken.
...
Bei CIFS-Volumes/Dateifreigaben sollte der Benutzer über die Berechtigung „Attribute schreiben“ verfügen, um sicherzustellen, dass die Zugriffszeiten der Dateien durch Data Classification-Scans nicht verändert werden. Wenn möglich, empfehlen wir, den in Active Directory konfigurierten Benutzer einer übergeordneten Gruppe in der Organisation zuzuordnen, die Berechtigungen für alle Dateien hat.
...
. Wählen Sie unter der Überschrift **Active Directory** die Option **Hinzufügen**.
. Geben Sie im Dialogfeld „Verbindung zu Active Directory herstellen“ die Active Directory-Details ein und wählen Sie dann *Verbinden*.
+
...
Wenn Sie Werte in Ihrer Active Directory-Integration ändern müssen, müssen Sie die Active Directory-Konfiguration in Data Classification aktualisieren. Kehren Sie zum Menü **Konfiguration** in Data Classification zurück und wechseln Sie dann zum Tab „Einstellungen“, um die Änderungen vorzunehmen.

Sie können die Integration auch löschen, indem Sie unter Active Directory die Schaltfläche **Entfernen** auswählen.
