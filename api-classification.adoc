...
[role="lead"]
Die NetApp Data Classification REST API bietet Zugriff auf dieselben Funktionen, die auch in der Web-UI verfügbar sind.
...
** Daten werden im JSON-Format exportiert (einfach zu parsen und an Drittanwendungen wie Splunk weiterzuleiten)


* Erstellen Sie Abfragen mit „UND“- und „ODER“-Anweisungen, schließen Sie Informationen ein und aus und mehr.
+
Beispielsweise können Dateien _ohne_ spezifische personenbezogene Informationen (PII) gefunden werden (diese Funktion ist in der Benutzeroberfläche nicht verfügbar). Bestimmte Felder können zudem vom Exportvorgang ausgeschlossen werden.
...
Um auf Swagger zuzugreifen, benötigen Sie die IP-Adresse Ihrer Data Classification Instanz. Für eine Cloud-Bereitstellung ist die öffentliche IP-Adresse zu verwenden. Anschließend ist folgender Endpunkt aufzurufen:
...
.API-Anforderung
Alle relevanten Felder und Optionen müssen für ein System abgerufen werden, um alle Filter auf der Registerkarte „Untersuchung“ anzuzeigen.
...
----
Die Antwort in den Anfrageparametern kann verwendet werden, um die Dateien zu filtern, die kopiert werden sollen.
...
Die Kopieraktion erstellen:

.API-Anforderung
Diese API ermöglicht das Erstellen mehrerer Aktionen.
